...
if(!empty($_POST['guardar'])){
    ...
    if (isset($_GET['opcBorrador'])) {
        ...
        if(file_exists('borradores/bo_'.$opc12.'.php')){
            header("Location: ../panel.php?ac=creador&ms=err&msm=elborradoryaestaenuso");
            exit;
        } else {
            $archivo = 'bo_'.$opc12;
        }
    ...
    if ($tipo=='borrador') {
    	...
    	if (file_exists($eliminar)) {
    		unlink($AC_DIRECTORIO."administracion/panel/creador/$eliminar");
