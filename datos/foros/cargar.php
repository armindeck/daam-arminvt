...
if(file_exists($AC_DIRECTORIO.$AC_CARPETA.'/datos/pubdatos.php')){
    require $AC_DIRECTORIO.$AC_CARPETA.'/datos/pubdatos.php';
    $comentario=array_reverse($comentario);
    $enlace_reacciones=$AC_DIRECTORIO.$AC_CARPETA."/datos/reacciones/";
    ...
    foreach($comentario as $item):
        ...
        $c_enlace='<a target="_blank" rel="nofollow" class="t12 link" href="'.$AC_DIRECTORIOs.'salir'.$AGREGAR_PHP.'?id='.$item['id'].'&carpeta='.$item['carpeta'].'&enlace='.$item['enlace'].'" title="Seguir a '.$item['enlace'].'">'.(substr($item['enlace'], 0, 25)).'...</a> <i class="fas fa-meteor forlinkcolor"></i>';
        ...
        $c_reportar='<span class="t12 repor"><a target="_blank" href="'.$AC_DIRECTORIOs.'reportar'.$AGREGAR_PHP.'?id='.$item['id'].'&carpeta='.$item['carpeta'].'">reportar</a></span>';
        #$c_reportar='<span class="t12 repor">reparando...</span>'; #QUITAR CUANDO REPARE LA SECCION DE REPORTAR
        $c_reacciones='<form method="post" action="'.$AC_DIRECTORIO.$AC_CARPETA.'/reac'.$AGREGAR_PHP.'?id='.$item['id'].'&dir='.$AC_DIRECTORIO.$AC_CARPETA.'/"> <input class="reaccion" type="submit" name="like" value="&#xf164; '.(file_get_contents($enlace_reacciones.'l'.$item['id'].'.txt')).'">
        ...
        $ex='Contador';
        $UbicacionArchivoContador=$AC_DIRECTORIO.$AC_CARPETA.'/datos/reportes/r'.$item['id'].'.txt'; $NoAumentarContador=true;
        ...
        if (isset(($item['imagen']))) {
            $texto_dividido=$dividir_texto.'<div class="flexCon"><hr><div class="m2"><a target="_blank" href="'.$AC_DIRECTORIOs.'salir'.$AGREGAR_PHP.'?id='.$item['id'].'&carpeta='.$item['carpeta'].'&enlace='.$item['enlace'].'"><div class="imagen"><img class="img1" src="'.$item['imagen'].'"></div></a></div></div>';
...
if(file_exists($archivo)){
    include $AC_DIRECTORIO.$AC_CARPETA.'/datos/pubdatos.php';
    $comentario=array_reverse($comentario);
    $enlace_reacciones=$AC_DIRECTORIO.$AC_CARPETA."/datos/reacciones/";
    foreach($comentario as $item):
