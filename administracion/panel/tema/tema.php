...
<p class="texini">Configuración avanzada <span class="t12"><?php echo file_get_contents(__DIR__.'/tema.x'); ?></span></p>

<form class="formulario" method="post">
	<select name="opcnombrearchivo">
    	<option class="optg">Ninguno</option>
        <?php
        $g_files = glob($AC_DIRECTORIO.'css/tema_*');
        function g_files_normalizar($string){
        	$string = str_replace('../../css/', '', $string);
            return $string;
        }
		foreach ($g_files as $key => $value) {
        	echo '<option '.(isset($opctema_nombre) ? (g_files_normalizar($value) == $opctema_nombre ? 'selected' : '') : '').'>'.g_files_normalizar($value).'</option>';
        }
        ?>
    </select>
...
<form method="post" action="actualizar.php">
	...
	<div class="flexCon">
		...
		<div class="formulario">
			...
			<input type="text" name="opctema_nombre" placeholder="tema_archivo.php" value="<?php if(isset($opctema_nombre)){ echo $opctema_nombre; } ?>"><hr>
